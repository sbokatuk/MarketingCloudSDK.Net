...
# Builds the device test app against the packed MarketingCloudSDK.Net package, installs it on an
# iOS simulator and runs its checks. The app prints its verdict to stdout; this script turns that
# into an exit code.
#
...
BUNDLE_ID="com.sbokatuk.mcnet.devicetests"
...
SIMULATOR_DEVICE="${SFMC_SIMULATOR_DEVICE:-iPhone 17}"

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PROJECT="${REPO_ROOT}/tests/MarketingCloudSDK.Net.DeviceTests/MarketingCloudSDK.Net.DeviceTests.csproj"
...
# The app's own intermediate output has to go too, not just the NuGet cache: neither restore nor
# the app packaging re-runs when the package version string is unchanged, so a rebuilt package of
# the same version would leave the previous build's assemblies in the app.
rm -rf "${REPO_ROOT}/tests/MarketingCloudSDK.Net.DeviceTests/obj" \
       "${REPO_ROOT}/tests/MarketingCloudSDK.Net.DeviceTests/bin"
...
# build AOT-compiles every assembly in the app, which means minutes of mono-aot-cross on several
# hundred BCL assemblies before the first check runs, and which has failed outright under memory
# pressure on public runners. Nothing is lost: this suite verifies that the packaging resolves
# the right binding underneath the façade and that the façade drives it - none of which AOT
# affects. The sample job builds a real MAUI app against the same packages and covers Release.
...
APP_PATH="$(find "${REPO_ROOT}/tests/MarketingCloudSDK.Net.DeviceTests/bin/Debug/${TARGET_FRAMEWORK}/${SIMULATOR_RID}" \
...
selection="$(xcrun simctl list devices available --json \
    | SFMC_PREFERRED_DEVICE="${SIMULATOR_DEVICE}" python3 -c "
import json, os, sys

preferred = os.environ['SFMC_PREFERRED_DEVICE']
...
# app terminates itself once it has printed SFMC_E2E_DONE.
set +e
...
if ! grep -q "SFMC_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    # binding dependency shows up here as a dyld failure naming the native framework.
    echo "==> no passing verdict; capturing crash output"
    xcrun simctl spawn "${udid}" log show --last 2m --predicate "process == 'MarketingCloudSDK.Net.DeviceTests'" \
        2>/dev/null | tail -100 | tee -a "${LOG_FILE}" || true
    echo "::error::MarketingCloudSDK.Net simulator checks failed or timed out"
    exit 1
fi
