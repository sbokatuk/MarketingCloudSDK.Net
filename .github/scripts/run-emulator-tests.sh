...
# Builds the device test app against the packed MarketingCloudSDK.Net package, installs it on a
...
PACKAGE_NAME="com.sbokatuk.mcnet.devicetests"
...
if ! grep -q "SFMC_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    echo "::error::MarketingCloudSDK.Net emulator checks failed or timed out"
    exit 1
fi
