...
#
# What is checked:
#   - every <PackageReference Include="MarketingCloudSDK..."/"SFMCSDK..." Version="..."> pin.
#     The suffix routes the expectation: .iOS/.Android pins are binding packages with their own
#     release lines, SFMCSDK.Net is the pinned core umbrella, everything else is this
#     repository's own packages at $(VersionPrefix).
#   - the device-check examples (run-simulator-tests.sh / run-emulator-tests.sh <version> ...).
#   - the "MarketingCloudSDK.Net.iOS <v>" / "MarketingCloudSDK.Net.Android <v>" prose mentions.
...
# Prose that explains the version *scheme* ("11.0.2.1 is MarketingCloudSDK 11.0.2, binding
# revision 1") is deliberately not checked - it describes the format, not the current release.
set -eu
...
IFS='
'
for pin in $(grep -oE 'Include="(MarketingCloudSDK|SFMCSDK)[^"]*" +Version="[0-9][^"]*"' "$readme"); do
  ...
  case "$id" in
    ...
    *.Android)  expected="$android_version" ;;
    SFMCSDK.Net) expected="$core_version" ;;
    *)          expected="$version" ;;
  esac
...
for mention in $(grep -oE 'MarketingCloudSDK\.Net\.(iOS|Android) +[0-9][0-9.]*' "$readme"); do
  ...
  if [ "$ver" != "$expected" ]; then
    echo "README shows MarketingCloudSDK.Net.$repo $ver, but the pinned binding version is $expected" >&2
    bad=1
  fi
done
IFS=$old_ifs
...
map_row="| $version | $native_version | $android_native_version | $ios_version | $android_version | $core_version |"
